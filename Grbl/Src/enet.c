...
static void report_options (bool newopt)
{
    ...
    if(newopt) {
//        hal.stream.write(1,",ETH",1);
        hal.stream.write_all(",ETH");
#if FTP_ENABLE
        if(services.ftp)
        	hal.stream.write(",FTP");
#endif
    } else {
        hal.stream.write_all("[IP:");
        hal.stream.write_all(IPAddress);
        hal.stream.write_all("]" ASCII_EOL);

        if(active_stream == StreamType_Telnet || active_stream == StreamType_WebSocket) {
            hal.stream.write_all("[NETCON:");
            hal.stream.write_all(active_stream == StreamType_Telnet ? "Telnet" : "Websocket");
            hal.stream.write_all("]" ASCII_EOL);
        }
    }
}
...
bool grbl_enet_start (void)
{
    if(nvs_address != 0) 
	{
		...
		ethernet.ip_mode = (ip_mode_t)IpMode_Static;
