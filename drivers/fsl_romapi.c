                                          ...
                                          const uint32_t *src)
{
	...
	uint32_t regPrimask;
	regPrimask = DisableGlobalIRQ();
	status = g_bootloaderTree->flexSpiNorDriver->program(instance, config, dstAddr, src);
	EnableGlobalIRQ(regPrimask);
    return status;
}
...
status_t ROM_FLEXSPI_NorFlash_Erase(uint32_t instance, flexspi_nor_config_t *config, uint32_t start, uint32_t length)
{
	...
	uint32_t regPrimask;
	regPrimask = DisableGlobalIRQ();
	status = g_bootloaderTree->flexSpiNorDriver->erase(instance, config, start, length);
	EnableGlobalIRQ(regPrimask);
    return status;
}
...
status_t ROM_FLEXSPI_NorFlash_EraseSector(uint32_t instance, flexspi_nor_config_t *config, uint32_t start)
{
	...
	uint32_t regPrimask;
	regPrimask = DisableGlobalIRQ();
	status = api_flexspi_nor_erase_sector(instance, config, start);
	EnableGlobalIRQ(regPrimask);
    return status;
}
...
status_t ROM_FLEXSPI_NorFlash_EraseAll(uint32_t instance, flexspi_nor_config_t *config)
{
	...
	uint32_t regPrimask;
	regPrimask = DisableGlobalIRQ();
	status = g_bootloaderTree->flexSpiNorDriver->erase_all(instance, config);
	EnableGlobalIRQ(regPrimask);
    return status;
}
...
status_t ROM_FLEXSPI_NorFlash_CommandXfer(uint32_t instance, flexspi_xfer_t *xfer)
{
	...
	uint32_t regPrimask;
	regPrimask = DisableGlobalIRQ();
	status = g_bootloaderTree->flexSpiNorDriver->xfer(instance, xfer);
	EnableGlobalIRQ(regPrimask);
    return status;
}
                                        ...
                                        uint32_t seqNumber)
{
	...
	uint32_t regPrimask;
	regPrimask = DisableGlobalIRQ();
	status = g_bootloaderTree->flexSpiNorDriver->update_lut(instance, seqIndex, lutBase, seqNumber);
	EnableGlobalIRQ(regPrimask);
    return status;
}
